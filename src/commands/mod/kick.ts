...
export default class KickCommand extends Command {
  ...
  public permissions = [<const>'KICK_MEMBERS'] as (PermissionString &
    'BOT_OWNER')[];
  ...
  public async run(message: Message, ...args: string[]): Promise<void> {
    ...
    if (!userMention.match(/<@!.*>/g)) {
      throw new Error(
        'The first argument in the kick command should be a user mention.',
      );
    }
    ...
    if (!user)
      throw new Error(
        'That user either does not exist or is not a member of this guild.',
      );
